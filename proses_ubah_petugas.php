...
if($_POST){
    ...
    if(empty($nama_petugas)){
        ...
        include "koneksi.php";
        $insert=mysqli_query($toko,"insert into petugas (nama_petugas, username, password, level) 
        value ('".$nama_petugas."','".$username."','".md5($password)."','".$level."')") or die(mysqli_error($toko));
        if($insert){
            echo "<script>alert('Sukses Merubah Data ');location.href='tambah_petugas.php';</script>";
        } else {
            echo "<script>alert('Gagal Merubah Data ');location.href='tambah_petugas.php';</script>";
        }
    }
}
?>